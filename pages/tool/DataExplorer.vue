...
export default {
  ...
  methods: {
    ...
    renderScatter() {
      ...
      getLatLonsForMap(brushGenerator, scatterParams, function(latLons) {
        vm.markers = [],
  ...
  mounted() {
    this.tableHeaders = [
      {text: 'Metro Area', value: 'MetroArea', align: 'left'},
      {text: this.varsMetaArr.filter(elem => elem.name === this.currentVars.x)[0].text, value: this.currentVars.x, align: 'right'},
      {text: this.varsMetaArr.filter(elem => elem.name === this.currentVars.y)[0].text, value: this.currentVars.y, align: 'right'},
      {text: this.varsMetaArr.filter(elem => elem.name === this.currentVars.radius)[0].text, value: this.currentVars.radius, align: 'right'},
      {text: this.varsMetaArr.filter(elem => elem.name === this.currentVars.color)[0].text, value: this.currentVars.color, align: 'right'}
    ];
    const filepath = '/data/data_explorer_12_6_17.csv';
    parseData(filepath).get((data) => {
      this.allData = numberify(data, varsMetaArr);
      this.allData.forEach(elem => elem.cbsaname15 = miscFunc.shortenMetroName(elem.cbsaname15))
